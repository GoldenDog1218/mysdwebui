...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        self.txt_box = gr.Textbox(label='选中的tags会自动追加到提示词里', interactive=True)  # adds text box
        self.txt_box.change(
            fn=self.textbox_changed,
            inputs=self.txt_box,
            outputs=self.txt_box
        )
        #self.weighting_chkbx = gr.Checkbox(label='增加注意括号',value=False)  # adds the attention checkbox
        ...
        #, self.weighting_chkbx
        self.component_list = [self.txt_box]
        if os.path.exists(folder_dir):  # now we pull out all the tags in the files
            with gr.Tabs(elem_id="tags_tabs") as self.tabs :
                #self.component_list.append(self.tabs)               
                txt_files_list = [f for f in listdir(folder_dir) if isfile(join(folder_dir, f))]
                if '最爱.txt' in txt_files_list:
                    index = txt_files_list.index('最爱.txt')
                ...
                if len(txt_files_list) > 0:
                    for file in txt_files_list:
                        if file.split('.')[-1] == 'txt':
                            ...
                            checkbox_choices = []
                            print("读取:"+txt_file_dir)
                            with open(txt_file_dir, encoding="utf-8") as f:
                                i=0
                                for line in f:
                                    i = i+1
                                    try:
                            ...
                            with gr.Tab(label=checkboxgroup_label) as self.tab:
