...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        input_path = gr.Textbox(label="输入文件路径", lines=1)
        ...
        with gr.Row():
            seed_walk = gr.Slider(
                minimum=-20, maximum=20, step=1, label="种子变化步长", value=0
            )
            seed_max_distance = gr.Slider(
                minimum=1, maximum=200, step=1, label="种子最大距离", value=10
            )

        with gr.Row():
            start_time = gr.Textbox(label="开始时间", value="00:00:00", lines=1)
            end_time = gr.Textbox(label="结束时间", value="00:00:00", lines=1)
